...
### DCTUserAdmin bulk import

The headless endpoint `POST /api/dct-useradmin/import` accepts `.csv`, `.xls`, and `.xlsx` files. Required columns are `id`, `inttype`, and `name`; documented Duck Creek entity columns are preserved: `intparentid`, `contact`, `phone`, `phoneext`, `address1`, `address2`, `city`, `state`, `zip`, and `reference`. Imports are upserts by `id` and return row-level validation errors.
